...
// Обработка клавиш установки параметров Ver.7.2
#include "DHT.h"
...
int bzzPin = 13;
      ...
      int h_a2 = 21; // Часы будильника 2
      int m_a2 = 15;  // Минуты будильника 2
      ...
      int eqel = 0; // флаг соотношения 0 =, 1 <, -1  > по температуре
      int al_temper = 0; // проверка всех условий по температуре
      
      int eqel_v =0; // флаг соотношения 0 =, 1 <, -1  > по температуре
      int al_vl = 0; // проверка всех условий по влажности
      
      int eqel_p =0; // флаг соотношения 0 =, 1 >, -1  < по температуре
...
void loop()
{
  ...
  if (eqel!=2)
  {
      ...
      if (cond == 0){
          if (al_temper>0&&al_vl>0) // Условие срабатывания второго будильника (уровень влажности и в перспективе давления)
          {
              if (alarm2 !=0)
