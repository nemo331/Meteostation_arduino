// Meteostation7_4 by Azat 
// Показывает дату время, температуру, влажность, давление
...
// Обработка клавиш установки параметров 
// Возможность не использовать датчики Ver.7.4
#include "DHT.h"
...
void setup()
{
  ...
  //rtc.setDate(15, 1, 2022);   // Set the date to August 6th, 2010
}
...
void loop()
{
  ...
  if (isnan(temp) || isnan(hum)||isnan(pr) ) // ошибка получения данных
  {
  //lcd.clear();
  lcd.setCursor(0,1);
              ...
              if (alarm2 !=0)
               { if ( t.hour == h_a2 && t.min == m_a2) // проверка будильника
                {digitalWrite (relay, HIGH) ;// и включение реле
                  ...
                  if(digitalRead (POT_5)== HIGH) {alarm2=0;}
                }
               }
          }
           else
           {
             ...
             {if ( t.hour == h_a && t.min == m_a) // проверка будильника
              {digitalWrite (relay, HIGH) ;// и включение реле
                ...
                if(digitalRead (POT_5)== HIGH) {alarm1=0;}
              }
             }
           }
        }
             ...
             {if ( t.hour == h_a && t.min == m_a) // проверка будильника
              {digitalWrite (relay, HIGH) ;// и включение реле
                ...
                if(digitalRead (POT_5)== HIGH) {alarm1=0;}
                  ...
                  if (alarm2 !=0)
                   { if ( t.hour == h_a2 && t.min == m_a2) // проверка будильника
                    {digitalWrite (relay, LOW) ;// и выключение реле
